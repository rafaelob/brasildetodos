"""Build a minimized CNES catalog from the bulk file linked by the official portal."""
...
def main():
    ...
    report = {'started_at': now(), 'source': URL, 'national_catalog_certified': False,
              'scope': 'CNES bulk, outpatient SUS eligibility; not live availability', 'stages': []}
    ...
    try:
        ...
        with zipfile.ZipFile(path) as archive:
            ...
            report['members'] = [{'name':i.filename,'bytes':i.file_size} for i in members]
            if len(members)!=1:
        ...
        rows = csv_records(path,member.filename,encoding,delimiter)
        report['import'] = import_places(database,rows,source,'cnes')
        ...
        report['partitions'] = [{'state':s,'records':n,'geocoded':g} for s,n,g in partitions]
