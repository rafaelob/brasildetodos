...
from test_resource_sync import contract,save_collection,PNCP,current


def test_all_record_errors_are_reported_while_nothing_is_published(database,tmp_path):
    root=tmp_path/'records'
    save_collection(root,[contract(1),contract(2,objetoContrato='X'),
    ...
    assert {i['rule'] for i in report['issues']}=={'string_too_short','too_long','blank'}
    assert 'SECRET' not in json.dumps(report)
    assert report['diagnostics'][0]['issues'][0]['constraints']=={'min_length':5}
...
def test_diagnostic_budget_never_hides_total_invalid_count(database,tmp_path,limit):
    root=tmp_path/'records';save_collection(root,[contract(n,objetoContrato='X') for n in range(1,5)])
