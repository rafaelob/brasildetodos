...
def create_app(database_url: str | None = None, *, testing: bool = False) -> FastAPI:
    ...
    app = FastAPI(title="Brasil de Todos", version=__version__, docs_url=None, redoc_url=None, openapi_url="/api/openapi.json", lifespan=lifespan)
    ...
    async def validation_error(_, error):
        return JSONResponse(status_code=422, content={"detail": "invalid_request", "errors": [{"loc": e["loc"], "type": e["type"]} for e in error.errors()]})
    ...
    def rate_limit(request: Request, purpose: str, limit: int, window: int = 900):
        ...
        with database.session() as session:
            ...
            statement = statement.on_conflict_do_update(index_elements=[RateBucket.id], set_={"count": RateBucket.count + 1}).returning(RateBucket.count)
    ...
    def config():
        ...
        if re.fullmatch(r"[0-9a-f]{40}", revision):
            code += "/tree/" + revision
        return {"registration_enabled": os.getenv("BDT_ALLOW_REGISTRATION") == "1", "source_code": code, "revision": revision, "photo_uploads": False}
    ...
    def login(body: Credentials, request: Request, response: Response):
        ...
        with database.session() as session:
            ...
            session.add(LoginSession(token_hash=hashlib.sha256(token.encode()).hexdigest(), user_id=user.id, expires_at=int(time.time()) + 28800))
    ...
    def places(q: str = Query("", max_length=200), kind: Literal["school", "health", "work"] | None = None,
               state: str | None = Query(None, pattern=r"^[A-Z]{2}$"), municipality_id: str | None = Query(None, pattern=r"^[0-9]{7}$"),
               bbox: str | None = Query(None, max_length=150), page: int = Query(1, ge=1, le=100000), limit: int = Query(30, ge=1, le=100)):
        ...
        if bbox:
            try:
                ...
                if not all(map(math.isfinite, (west, south, east, north))) or not (-180 <= west < east <= 180 and -90 <= south < north <= 90):
                    raise ValueError()
            except ValueError:
    ...
    def place(place_id: str):
        with database.session() as session:
            ...
            observations = session.scalars(select(Observation).where(Observation.place_id == place_id, Observation.status == "approved").order_by(Observation.created_at.desc()).limit(100))
            return {"place": row.payload, "finance": [item.payload for item in money], "observations": [public_observation(item) for item in observations]}
    ...
    def region(municipality_id: str):
        with database.session() as session:
            ...
            return {"total": total, "truncated": total > 1000, "municipality": {"id": row.id, "name": row.name, "state": row.state}, "events": events, "cells": financial_cells(events), "limit": 1000, "scope": "loaded_records_only"}

    @app.get("/api/coverage")
    def coverage():
        with database.session() as session:
            grouped = session.execute(select(Place.dataset, Place.state, func.count(Place.id), func.count(Place.latitude)).where(Place.catalogue_eligible.is_(True)).group_by(Place.dataset, Place.state)).all()
            ...
            return {"national_catalog_certified": False, "note": "Counts describe loaded records, not complete national coverage.",
                    ...
                    "runs": [{"id": r.id, "dataset": r.dataset, "status": r.status, "counts": r.counts, "source": r.source, "finished_at": r.finished_at} for r in runs]}
    ...
    def mine(user=Depends(current_user)):
        with database.session() as session:
            rows = session.scalars(select(Observation).where(Observation.author_id == user["id"]).order_by(Observation.created_at.desc()).limit(100))
    ...
    def review(observation_id: str, body: Review, user=Depends(reviewer)):
        with database.session() as session:
            ...
            changed = session.execute(update(Observation).where(Observation.id == observation_id, Observation.status == "pending").values(status=body.decision, reviewer_id=user["id"], reviewed_at=now(), review_note=body.note))
