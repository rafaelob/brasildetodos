...
type Summary={municipality:Town;generated_at:string;service_records:number;resource_records:number;services:{kind:string;records:number;with_geometry:number;without_geometry:number}[];resource_groups:{source_id:string;territorial_basis:string;records:number}[];financial_event_groups?:{phase:string;records:number}[]};
...
function PublicDensityDashboard({services,t,locale}:{services:Summary['services'];t:(key:string)=>string;locale:Locale}){
 if(!services||services.length===0)return null;
 const totalFacilities=services.reduce((acc,s)=>acc+s.records,0);
 return <div className="region-audit-dashboard panel">
  ...
  <div className="region-audit-grid">{services.map(item=>{
   const geoPct=item.records>0?Math.round((item.with_geometry/item.records)*100):0;
   const densityPct=totalFacilities>0?Math.round((item.records/totalFacilities)*100):0;
   return <div className="region-audit-item" key={item.kind}>
    <div className="region-audit-labels"><strong>{t(item.kind)}</strong><span>{densityPct}% {t('regionDensityShare')}</span></div>
    <div className="region-progress-track" role="progressbar" aria-valuenow={geoPct} aria-valuemin={0} aria-valuemax={100} title={`${geoPct}% ${t('regionGeoRate')}`}>
     <div className="region-progress-fill" style={{width:`${geoPct}%`}}></div>
    </div>
    <div className="region-audit-sub">
     <small>{item.records.toLocaleString(locale)} {t('regionRecords')} ({densityPct}%)</small>
     <small><strong>{geoPct}%</strong> {t('regionGeoRate')}</small>
    </div>
   </div>;
  })}</div></div>;
}

function ResourceDistribution({groups,total,financialGroups,t,locale}:{groups:Summary['resource_groups'];total:number;financialGroups?:Summary['financial_event_groups'];t:(key:string)=>string;locale:Locale}){
 if((!groups||groups.length===0||total===0)&&(!financialGroups||financialGroups.length===0))return null;
 const bySource:Record<string,number>={};
 if(groups)for(const g of groups)bySource[g.source_id]=(bySource[g.source_id]||0)+g.records;
 ...
 return <div className="region-resource-dist panel">
  ...
  {entries.length>0&&<><div className="region-stacked-bar">{entries.map(([src,count])=>{
  ...
  {financialGroups&&financialGroups.length>0&&<div className="region-finance-phases-box">
   <h5>{t('regionFinancePhasesTitle')}</h5><p className="quiet">{t('regionFinancePhasesNote')}</p>
   <div className="region-finance-phase-chips">{financialGroups.map(fg=><span className="pill status-pill" key={fg.phase}>
    <strong>{t('phase_'+fg.phase)||fg.phase}</strong>: {fg.records.toLocaleString(locale)} {t('regionRecords')}
   </span>)}</div>
  </div>}
 </div>;
}
