...
export default function Workbench({t,locale='pt-BR'}:{t:Translator;locale?:Locale}){
  ...
  const [candidatesList,setCandidatesList]=useState<Candidate[]>([]),[pageRoute,setPageRoute]=useState<string>(''),[hasOcrText,setHasOcrText]=useState(false),[excerptInput,setExcerptInput]=useState<string>('');
  ...
  function register(event:FormEvent<HTMLFormElement>,kind:'documents'|'resources'){
    ...
    execute(async()=>{const common={title:String(values.get('title')),source:sourceFrom(values)};
      ...
      if(kind==='documents'){setDocId(result.id);setPageText(null);setCandidatesList([]);setPageRoute('');setHasOcrText(false);}form.reset();});
  }
  async function openPage(){setBusy(true);setMessage('');setPageText(null);setCandidatesList([]);setPageRoute('');setHasOcrText(false);try{const result=await api<{page:{text:string;ocr_candidate_text?:string;route?:string;candidates?:Candidate[];ocr_candidates?:Candidate[]}}>('/workbench/documents/'+encodeURIComponent(docId)+'/pages/'+page);setPageText([result.page.text,result.page.ocr_candidate_text].filter(Boolean).join('\n\n'));setPageRoute(result.page.route||'native');setHasOcrText(Boolean(result.page.ocr_candidate_text));setCandidatesList([...(result.page.candidates||[]),...(result.page.ocr_candidates||[])]);}catch{setMessage('missingExtraction');}finally{setBusy(false);}}
  ...
  return <section className="page"><h1>{t('workbench')}</h1><p className="callout">{t('extractionOperator')}</p><p>{t('independentReview')}</p>{message&&<p role="status">{t(message)}</p>}
    <div className="detail-columns"><div>
      ...
      <article className="panel"><div className="workbench-panel-header"><h2>{t('sourceRestricted')}</h2>{pageRoute&&<span className={'route-badge route-'+pageRoute}>{hasOcrText?t('routeOcrProcessed'):pageRoute==='ocr_candidate'?t('routeOcrCandidate'):pageRoute==='review_encoding'?t('routeReviewEncoding'):t('routeNative')}</span>}</div><label htmlFor="selected-document">{t('selectedDocument')}</label><select id="selected-document" value={docId} onChange={event=>{setDocId(event.target.value);setPage(1);setPageText(null);setCandidatesList([]);setPageRoute('');setHasOcrText(false);}}><option value="">{t('selectedDocument')}</option>{documents.map(doc=><option key={doc.id} value={doc.id}>{doc.title} · {doc.pages}</option>)}</select><label>{t('pageNumber')}<input type="number" min={1} max={1000} value={page} onChange={event=>{setPage(Number(event.target.value));setPageText(null);setCandidatesList([]);setPageRoute('');setHasOcrText(false);}}/></label><button disabled={!docId||busy||page<1} onClick={openPage}>{t('loadPage')}</button>{candidatesList.length>0&&<div className="candidates-container"><h3>{t('extractedCandidates')}</h3><div className="candidates-list">{candidatesList.map((c,i)=><div key={i} className="candidate-card"><span className="candidate-badge">{fieldLabel(c.field,t)}</span><strong className="candidate-text">{c.raw||String(c.value)}</strong><button type="button" className="candidate-apply-btn" onClick={()=>setExcerptInput(c.raw||String(c.value))} title={t('useAsExcerpt')}>↳ {t('useAsExcerpt')}</button></div>)}</div></div>}{pageText!==null&&<pre className="extracted-text" aria-label={t('sourceRestricted')}>{pageText}</pre>}</article>
