...
export default function Workbench({t}:{t:Translator}){
  ...
  function register(event:FormEvent<HTMLFormElement>,kind:'documents'|'resources'){
    ...
    execute(async()=>{const common={title:String(values.get('title')),source:sourceFrom(values)};
      ...
      if(kind==='documents'){setDocId(result.id);setPageText(null);}form.reset();});
  }
  async function openPage(){setBusy(true);setMessage('');setPageText(null);try{const result=await api<{page:{text:string;ocr_candidate_text?:string}}>('/workbench/documents/'+encodeURIComponent(docId)+'/pages/'+page);setPageText([result.page.text,result.page.ocr_candidate_text].filter(Boolean).join('\n\n'));}catch{setMessage('missingExtraction');}finally{setBusy(false);}}
  function propose(event:FormEvent<HTMLFormElement>){event.preventDefault();const form=event.currentTarget,values=new FormData(form);execute(async()=>{await api('/workbench/links',{method:'POST',body:JSON.stringify({place_id:values.get('place_id'),resource_id:values.get('resource_id'),document_id:docId,page,excerpt:values.get('excerpt'),justification:values.get('justification')})});form.reset();});}
  ...
  return <section className="page"><h1>{t('workbench')}</h1><p className="callout">{t('extractionOperator')}</p><p>{t('independentReview')}</p>{message&&<p role="status">{t(message)}</p>}
    <div className="detail-columns"><div>
      ...
      <article className="panel"><h2>{t('sourceRestricted')}</h2><label>{t('selectedDocument')}<select value={docId} onChange={event=>{setDocId(event.target.value);setPage(1);setPageText(null);}}><option value="">{t('selectedDocument')}</option>{documents.map(doc=><option key={doc.id} value={doc.id}>{doc.title} · {doc.pages}</option>)}</select></label><label>{t('pageNumber')}<input type="number" min={1} max={1000} value={page} onChange={event=>{setPage(Number(event.target.value));setPageText(null);}}/></label><button disabled={!docId||busy||page<1} onClick={openPage}>{t('loadPage')}</button>{pageText!==null&&<pre className="extracted-text" aria-label={t('sourceRestricted')}>{pageText}</pre>}</article>
    </div><div><form className="panel" onSubmit={propose}><h2>{t('proposeLink')}</h2><label>{t('placeIdentifier')}<input name="place_id" required maxLength={180}/></label><label>{t('resourceID')}<input name="resource_id" list="known-resources" required maxLength={200}/><datalist id="known-resources">{resources.map(resource=><option key={resource.id} value={resource.id}>{resource.title}</option>)}</datalist></label><label>{t('excerpt')}<textarea name="excerpt" required minLength={10} maxLength={1500}/></label><label>{t('justification')}<textarea name="justification" required minLength={20} maxLength={2000}/></label><p>{t('selectedDocument')}: <code>{docId||t('unknown')}</code> · {t('pageNumber')} {page}</p><button className="primary" disabled={busy||!docId||pageText===null}>{t('proposeLink')}</button></form></div></div>
    <h2>{t('reviewAction')}</h2><label>{t('reviewAction')}<select value={status} onChange={event=>setStatus(event.target.value)}>{['candidate','reviewed','rejected','retracted'].map(value=><option key={value} value={value}>{t(value)}</option>)}</select></label><button onClick={()=>execute(async()=>{})} disabled={busy}>{t('refresh')}</button>
    {queue.map(row=><article className="panel" key={row.id}><p><code>{row.place_id}</code> → <code>{row.resource_id}</code></p><blockquote>{row.excerpt}</blockquote><p>{row.justification}</p><p>{t('pageNumber')}: {row.page}</p>{row.can_review&&['candidate','reviewed'].includes(row.status)?<form onSubmit={event=>review(event,row)}><label>{t('reviewAction')}<select name="decision">{row.status==='reviewed'?<option value="retracted">{t('retracted')}</option>:<><option value="reviewed">{t('reviewed')}</option><option value="rejected">{t('rejected')}</option></>}</select></label><label>{t('reviewNote')}<textarea name="note" required minLength={20} maxLength={2000}/></label>{row.status==='candidate'&&<label className="check"><input type="checkbox" name="checked"/>{t('publicExcerptCheck')}</label>}<button disabled={busy} className="primary">{t('submit')}</button></form>:<p>{t('independentReview')}</p>}</article>)}
  </section>;
}
