...
function ImportHistory({locale,t}:{locale:Locale;t:T}){
 ...
 return <section className="coverage-history" aria-labelledby="import-history-title"><h2 id="import-history-title">{t('coverageImports')}</h2>
  ...
  <div className="coverage-filters"><label>{t('coverageImportSource')}<select value={source} onChange={e=>{setSource(e.target.value);setPage(1);}}><option value="all">{t('coverageAll')}</option>{SOURCE_IDS.map(id=><option value={id} key={id}>{sourceLabel(id,t)}</option>)}</select></label>
  <label>{t('coverageImportStatus')}<select value={status} onChange={e=>{setStatus(e.target.value);setPage(1);}}><option value="all">{t('coverageAll')}</option>{STATUS_IDS.map(id=><option value={id} key={id}>{statusLabel(id,t)}</option>)}</select></label></div>
 ...
 return <section className="page coverage-page" aria-labelledby="coverage-title">
  ...
  {data&&<><p className="quiet">{t('coverageObservedAt')}: <time dateTime={data.generated_at}>{watchDate(data.generated_at,locale,t)}</time></p>
   ...
   <details className="coverage-territory"><summary>{t('coverageMore')}</summary><h2>{t('coverageTerritory')}</h2>
    <div className="coverage-filters"><label>{t('coverageSourceFilter')}<select value={source} onChange={e=>setSource(e.target.value)}><option value="all">{t('coverageAll')}</option>{SOURCE_IDS.map(id=><option key={id} value={id}>{sourceLabel(id,t)}</option>)}</select></label>
    <label>{t('coverageStateFilter')}<select value={state} onChange={e=>setState(e.target.value)}><option value="all">{t('coverageAll')}</option>{STATE_IDS.map(id=><option key={id}>{id}</option>)}<option value="unknown">{t('coverageUnknownState')}</option></select></label></div>
    {rows.length?<div className="table-scroll"><table><caption>{t('coverageTerritory')}</caption><thead><tr>{['source','states','coverageRecords','coverageLocated','coverageUnlocated'].map(key=><th key={key} scope="col">{t(key)}</th>)}</tr></thead><tbody>{rows.map((row:Partition)=><tr key={row.source_id+row.state}><th scope="row">{sourceLabel(row.source_id,t)}</th><td>{row.state==='unknown'?t('coverageUnknownState'):row.state}</td><td>{coverageCount(row.records,locale,t)}</td><td>{coverageCount(row.geocoded,locale,t)}</td><td>{coverageCount(row.without_geometry,locale,t)}</td></tr>)}</tbody></table></div>:<p>{t('coverageNoPartitions')}</p>}
   </details>
  </>}
