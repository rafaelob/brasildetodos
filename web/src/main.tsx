...
function App(){
  ...
  function formatDataset(ds:string){
    if(ds==='cnes'||ds==='cnes_establishments')return t('sourceCnes');
    if(ds==='inep'||ds==='inep_schools')return t('sourceInep');
    if(ds==='transferegov'||ds==='transferegov_proposals'||ds==='transferegov_instruments')return t('sourceTransferegov');
    if(ds==='pncp'||ds==='pncp_contracts')return t('sourcePncp');
    return ds;
  }
  function SourceView({source}:{source:Source}){const url=safeReference(source.url);return <div className="source"><span className="eyebrow">{t('source')}</span><strong>{formatDataset(source.dataset)}</strong><dl><dt>{t('reference')}</dt><dd>{source.reference_date||t('unknown')}</dd><dt>{t('collected')}</dt><dd>{new Date(source.collected_at).toLocaleDateString(locale)}</dd></dl>{url&&<a href={url} target="_blank" rel="noopener noreferrer">{t('sourceOriginal')} ↗</a>}<details><summary>SHA-256</summary><code>{source.snapshot_sha256}</code></details></div>;}
  ...
  function Cards({items}:{items:Place[]}){
    ...
    return <div className="place-list">{items.map(place=><article className="place-card" key={place.id}><div className={'kind-icon '+place.kind} aria-hidden="true">{place.kind==='school'?'▤':place.kind==='health'?'+':'◇'}</div><div><div className="card-top-meta"><span className="eyebrow">{t(place.kind)} · {place.state}</span>{place.latitude!==null?<span className="pill geo-pill">⌖ {t('geocodedBadge')}</span>:<span className="pill quiet-pill">⊘ {t('notGeocodedBadge')}</span>}</div><h2><button className="text-button" onClick={()=>select(place.id)}>{place.name}</button></h2><p>{place.address||t('noAddress')}</p>{place.phone&&<p className="card-phone">{t('contactNumber')}: {place.phone}</p>}{place.declared_services.length>0&&<div className="card-services">{place.declared_services.slice(0,3).map(s=><span className="pill service-pill" key={s}>{t(s)}</span>)}</div>}<div className="card-bottom-bar"><small>{formatDataset(place.source.dataset)} · {place.source.reference_date||t('unknown')}</small><div style={{display:'flex',gap:'.4rem',alignItems:'center'}}><button className="copy-pill" title={t('copyId')} aria-label={t('copyId')} onClick={()=>copyPlaceId(place.id)}>📋 {place.id.split(':')[1]||place.id}</button>{place.latitude!==null&&place.longitude!==null&&<button className="btn-map-3d" onClick={()=>focusPlace3D(place)} title={t('viewOnMap')} aria-label={t('viewOnMap')}>◩ {t('viewOnMap')}</button>}</div></div></div><button className="save-icon" aria-label={t(favorites.includes(place.id)?'unsave':'save')+' '+place.name} aria-pressed={favorites.includes(place.id)} onClick={()=>favorite(place.id)}>{favorites.includes(place.id)?'★':'☆'}</button></article>)}</div>;
  }
