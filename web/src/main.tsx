...
function App(){
  ...
  function Cards({items}:{items:Place[]}){return <div className="place-list">{items.map(place=><article className="place-card" key={place.id}><div className={'kind-icon '+place.kind} aria-hidden="true">{place.kind==='school'?'▤':place.kind==='health'?'+':'◇'}</div><div><span className="eyebrow">{t(place.kind)} · {place.state}</span><h2><button className="text-button" onClick={()=>select(place.id)}>{place.name}</button></h2><p>{place.address||t('noAddress')}</p><small>{place.source.dataset} · {place.source.reference_date||t('unknown')}</small>{place.latitude===null&&<small className="quiet">{t('noGeo')}</small>}</div><button className="save-icon" aria-label={t(favorites.includes(place.id)?'unsave':'save')+' '+place.name} aria-pressed={favorites.includes(place.id)} onClick={()=>favorite(place.id)}>{favorites.includes(place.id)?'★':'☆'}</button></article>)}</div>;}
