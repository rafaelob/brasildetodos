...
import {coverageCount,heroFromCoverage} from './coverage-text.mjs';
...
type HeroCoverage={status:'loading'}|{status:'error'}|{status:'ready';payload:unknown};
...
function App(){
  ...
  const [heroCoverage,setHeroCoverage]=useState<HeroCoverage>({status:'loading'});
  ...
  useEffect(()=>{const controller=new AbortController();
    api<unknown>('/coverage',{signal:controller.signal}).then(payload=>{if(!controller.signal.aborted)setHeroCoverage({status:'ready',payload});}).catch(()=>{if(!controller.signal.aborted)setHeroCoverage({status:'error'});});
  ...
  async function navigate(next:string){if(parseResourceRoute(window.location.hash)){window.history.replaceState(null,'',window.location.pathname+window.location.search);setResourceRoute('');}setView(next);setDetail(null);setNotice('');cancelDetails();
    if(next!=='account')setRecoveryCode('');
  ...
  function ObservationCards({rows,allowContest}:{rows:Observation[];allowContest?:boolean}){return <div className="observations">{rows.map(row=><article key={row.id}><span className="pill">{t(row.status==='pending'?'pendingLabel':row.status==='withdrawn'?'observationWithdrawn':row.status)}</span>{row.contested&&<span className="pill">{t('contested')}</span>}<p className="observation-body">{row.observation.body}</p><ObservationDownloads row={row} locale={locale}/><small>{t(row.observation.mode)} · {row.observation.observed_on}</small>{safeReference(row.observation.reference_url)&&<p><a href={safeReference(row.observation.reference_url)!} target="_blank" rel="noopener noreferrer">{t('referenceUrl')} ↗</a></p>}{allowContest&&row.status==='rejected'&&<form className="panel" onSubmit={async event=>{event.preventDefault();const data=new FormData(event.currentTarget);setBusy(true);try{await api('/observations/'+row.id+'/contest',{method:'POST',body:JSON.stringify({note:data.get('note')})});setNotice('contestSent');setMine(await api<Observation[]>('/observations/mine'));}catch{failure();}finally{setBusy(false);}}}><p>{t('contestNote')}</p><label>{t('contestReason')}<textarea name="note" minLength={20} maxLength={1000} required rows={3}/></label><button className="primary" disabled={busy}>{t('contestSubmit')}</button></form>}</article>)}</div>;}
  function ExploreHero(){
    const hero=heroCoverage.status==='ready'?heroFromCoverage(heroCoverage.payload):null;
    return <section className="hero">
      <span className="eyebrow">{t('heroEyebrow')}</span>
      <h1>{t('tagline')}</h1>
      <p>{t('intro')}</p>
      <p className="callout">{t('coverageScope')}</p>
      {hero?.emptyInstall&&<p className="callout">{t('noneLoaded')}</p>}
      <div aria-live="polite" role="status">{heroCoverage.status==='loading'?t('loading'):heroCoverage.status==='error'?t('coverageError'):null}</div>
      <div className="hero-stats" aria-busy={heroCoverage.status==='loading'}>
        {hero&&<>
  ...
  {tab==='finance'&&<div role="tabpanel" id="tabpanel-finance"><p className="callout">{t('regionNote')}</p><MoneyCards events={detail.finance}/><PlaceEvidence placeId={current.id} t={t} locale={locale}/><button onClick={()=>openRegion(current.municipality_id)}>{t('region')} →</button></div>}
  ...
  {view==='account'&&<section className="page narrow"><h1>{t('account')}</h1><p>{t('authNote')}</p>{user?<><p><strong>{user.username}</strong></p><button onClick={async()=>{try{await api('/auth/logout',{method:'POST'});setUser(null);setMine([]);setRecoveryCode('');}catch{failure();}}}>{t('logout')}</button><h2>{t('recoveryTitle')}</h2><p>{t('recoveryPrepareNote')}</p>{recoveryCode&&<p className="callout"><strong>{t('recoveryShowOnce')}</strong><code style={{display:'block',marginTop:'0.5rem'}}>{recoveryCode}</code></p>}<form className="panel" onSubmit={async event=>{event.preventDefault();const form=event.currentTarget,data=new FormData(form);setBusy(true);try{const result=await api<{code:string}>('/auth/recovery/prepare',{method:'POST',body:JSON.stringify({password:data.get('password')})});setRecoveryCode(result.code);setNotice('recoveryPrepared');form.reset();}catch{failure();}finally{setBusy(false);}}}><label>{t('recoveryCurrentPassword')}<input name="password" type="password" autoComplete="current-password" minLength={12} maxLength={128} required/></label><button className="primary" disabled={busy}>{t('recoveryGenerate')}</button></form><form className="panel" onSubmit={async event=>{event.preventDefault();const form=event.currentTarget,data=new FormData(form);setBusy(true);try{await api('/auth/recovery/revoke',{method:'POST',body:JSON.stringify({password:data.get('password')})});setRecoveryCode('');setNotice('recoveryRevoked');form.reset();}catch{failure();}finally{setBusy(false);}}}><label>{t('recoveryCurrentPassword')}<input name="password" type="password" autoComplete="current-password" minLength={12} maxLength={128} required/></label><button disabled={busy}>{t('recoveryRevoke')}</button></form><h2>{t('myContributions')}</h2><ObservationCards rows={mine} allowContest/><PrivacyControls rows={mine} t={t} onChange={()=>{api<Observation[]>('/observations/mine').then(setMine).catch(failure);}} onDeleted={()=>{setUser(null);setMine([]);setRecoveryCode('');}}/></>:<><form className="panel" onSubmit={authentication}><label>{t('username')}<input name="username" autoComplete="username" pattern="[a-zA-Z0-9_-]{3,40}" required/></label><label>{t('password')}<input name="password" type="password" autoComplete="current-password" minLength={12} maxLength={128} required/></label><button className="primary" disabled={busy}>{t('login')}</button>{config?.registration_enabled?<button disabled={busy} type="button" onClick={event=>{const form=event.currentTarget.form;if(form?.reportValidity())authentication({preventDefault(){},currentTarget:form} as React.FormEvent<HTMLFormElement>,true);}}>{t('register')}</button>:<p>{t('registrationOff')}</p>}</form><h2>{t('recoveryTitle')}</h2><form className="panel" onSubmit={async event=>{event.preventDefault();const form=event.currentTarget,data=new FormData(form);if(data.get('new_password')!==data.get('new_password_confirm')){setNotice('recoveryMismatch');return;}setBusy(true);setNotice('');try{await api('/auth/recovery/consume',{method:'POST',body:JSON.stringify({username:data.get('username'),code:data.get('code'),new_password:data.get('new_password')})});setNotice('recoveryReset');form.reset();}catch{failure();}finally{setBusy(false);}}}><label>{t('username')}<input name="username" autoComplete="username" pattern="[a-zA-Z0-9_-]{3,40}" required/></label><label>{t('recoveryCode')}<input name="code" autoComplete="off" minLength={16} maxLength={128} required/></label><label>{t('recoveryNewPassword')}<input name="new_password" type="password" autoComplete="new-password" minLength={12} maxLength={128} required/></label><label>{t('recoveryConfirmPassword')}<input name="new_password_confirm" type="password" autoComplete="new-password" minLength={12} maxLength={128} required/></label><button className="primary" disabled={busy}>{t('recoveryUse')}</button></form></>}</section>}
  ...
  {view==='workbench'&&user?.role==='reviewer'&&<Workbench t={t} locale={locale}/>}
  </>}
